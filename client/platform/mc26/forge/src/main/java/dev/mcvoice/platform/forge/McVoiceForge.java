...
/** Forge entry point for Minecraft 26.x (client-only mod). */
@Mod("mcvoice")
public final class McVoiceForge {
    ...
    public McVoiceForge(FMLJavaModLoadingContext context) {
        ...
        AddGuiOverlayLayersEvent.BUS.addListener(e -> e.getLayeredDraw().add(Identifier.fromNamespaceAndPath("mcvoice", "hud"),
            (graphics, delta) -> {
                if (client != null) {
                    client.renderHud(new Mc26Canvas(graphics));
                }
            }));
    ...
    static final class ForgeSvcChannels implements SimpleVoiceChatAdapter {
        ...
        ForgeSvcChannels() {
            for (String ch : SvcChannelNames.CLIENTBOUND) {
                EventNetworkChannel c = ChannelBuilder.named(Identifier.parse(ch)).optional().eventNetworkChannel();
                c.addListener(event -> {
            ...
            for (String ch : SvcChannelNames.SERVERBOUND) {
                channels.put(ch, ChannelBuilder.named(Identifier.parse(ch)).optional().eventNetworkChannel());
            }
        }
