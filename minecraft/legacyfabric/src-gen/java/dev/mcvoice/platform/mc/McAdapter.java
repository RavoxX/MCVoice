...
public final class McAdapter implements MinecraftAdapter {
    ...
    private final GuiAdapter gui = new GuiAdapter() {
        ...
        public int[] chatPointer() {
            ...
            if (!(m.currentScreen instanceof net.minecraft.client.gui.screen.ChatScreen)) {
                return null;
            }
            McCanvas c = new McCanvas();
            int x = org.lwjgl.input.Mouse.getX() * c.width() / Math.max(1, m.width);
            int y = c.height() - org.lwjgl.input.Mouse.getY() * c.height() / Math.max(1, m.height) - 1;
