...
pub fn scope_key(network_id: &str, attested: &str, world_id: &str) -> String {
    if attested.is_empty() {
        format!("{network_id}|{world_id}")
    } else {
        format!("attested:{attested}|{world_id}")
    }
}
...
pub fn deliver(cfg: &RoutingConfig, now_ms: i64, s: &Peer, r: &Peer, mode: u8) -> bool {
    ...
    if r.scope_key != s.scope_key || !r.in_world || !r.udp_verified || !r.authenticated || r.deafened {
        return false;
    }
    let (Some(sp), Some(rp)) = (s.fresh_pos(cfg, now_ms), r.fresh_pos(cfg, now_ms)) else {
        return false;
    };
    if !r.visible.contains(&s.uuid) {
        return false;
    }
    if cfg.require_mutual && !s.visible.contains(&r.uuid) {
        return false;
    }
